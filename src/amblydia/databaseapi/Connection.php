...
final class Connection {
     ...
     * @throws Exception
     */
    public function __construct(private array $credentials, private readonly AmbFeature $feature) {
        ...
        switch ($type) {
            case "sqlite":
            case "sqlite3":
                $this->credentials = ["path" => $feature->getDataPath() . ($config["path"] ?? "db.sqlite")];
                $path = $feature->getRootDataPath() . "sqlite.sql";
                $this->type = "SQLite3";
                break;

            case "mysql":
                $this->credentials = $config;
                $path = $feature->getRootDataPath() . "mysql.sql";
                $this->type = "MySQL";
                break;
        ...
        foreach ($this->queries as $queryId => $data) {
            if (str_contains($queryId, "__init")) {
                $this->executeRaw($this->queries[$queryId]["query"], function ($result) use ($feature): void {
                    if ($result instanceof Exception) {
                        if ($result instanceof PDOException) {
                            $feature->getLoader()->getLogger()->error($result->getMessage());

                            return;
                        }

                        $feature->getLoader()->getLogger()->logException($result);
                    }
                });
            }
        }
     ...
     * @return AmbFeature
     */
    public function getFeature(): AmbFeature {
        return $this->feature;
    }
    ...
    public function batchExecute(array $queries, ?callable $onComplete = null): void {
        $this->getFeature()->getLoader()->getServer()->getAsyncPool()->submitTask(
            new GenericQueryTask($this->credentials, $queries, function ($result) use($onComplete): void {
                if (is_string($result)) {
                    $this->getFeature()->getLoader()->getLogger()->error($result);
